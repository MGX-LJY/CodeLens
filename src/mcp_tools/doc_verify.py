...
class DocVerifyTool:
    ...
    def _verify_missing_files(self, project_path: str, expected_files: List[str]) -> Dict[str, Any]:
        ...
        return self._success_response({
            ...
            "completion_percentage": round((len(existing_files) / len(expected_files)) * 100, 2) if expected_files else 100
        })
        ...
        for layer_name, layer_info in status.get("layer_status", {}).items():
            ...
            layer_details[layer_name] = {
                ...
                "file_details": {
                    ...
                    "latest_modified": max(f.get("modified", "") for f in files if f.get("modified")) if files else None,
